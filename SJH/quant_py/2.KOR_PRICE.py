...
for i in tqdm(range(0, len(ticker_list))):
    ...
    # 시작일과 종료일
    fr = (date.today() + relativedelta(months=-2)).strftime("%Y%m%d")
    to = (date.today()).strftime("%Y%m%d")

    # 오류 발생 시 이를 무시하고 다음 루프로 진행
    # https://fchart.stock.naver.com/siseJson.nhn?symbol=005930&requestType=1&startTime=20240101&endTime=20240129&timeframe=day
    try:
